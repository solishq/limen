...
describe('Phase 8: Dependency Boundary', () => {
  ...
  it('DC-P8-601: Phase 8 did not introduce migration files (Phase 9 added 033)', () => {
    ...
    assert.ok(
      lastMig.includes('050') || lastMig.includes('049') || lastMig.includes('048') || lastMig.includes('047') || lastMig.includes('046') || lastMig.includes('036') || lastMig.includes('035') || lastMig.includes('034') || lastMig.includes('033') || lastMig.includes('032'),
      `Last migration should be 032-050 (Phase 5 through Phase 12 + fixes + lifecycle + remediation + output governance). Got: ${lastMig}`,
    );
  });
});
