/**
 * Groq API adapter (OpenAI-compatible Chat Completions format).
 * S ref: §25.4 (LLM Gateway), I-01 (no SDK deps), I-04 (provider independence)
 *
 * Phase: 5C (Sprint 5C — Ollama + Groq + Mistral Adapters)
 * Implements: ProviderAdapter for Groq's OpenAI-compatible Chat Completions API.
 *
 * Groq speaks the OpenAI Chat Completions wire format — this is a thin wrapper
 * around openai_compat.ts shared functions. Minimal adapter.
 *
 * Auth: Authorization: Bearer {key} — same as OpenAI
 * URL: {baseUrl}/openai/v1/chat/completions (default baseUrl: https://api.groq.com)
 ...
 * Deliberation: Always estimated with 0 tokens (Groq does not report reasoning tokens)
 *
 * Security:
...
/** Retryable HTTP status codes for Groq */
const RETRYABLE_STATUS_CODES = new Set([429, 500, 502, 503]);

// ─── Groq Adapter Factory ───

/**
 * Create a Groq API adapter.
 *
 * @param providerId - Provider identifier for health tracking
 * @param baseUrl - Base URL for the Groq API (e.g., 'https://api.groq.com')
...
export function createGroqAdapter(
  ...
  function parseStream(
    ...
    // Groq does not report reasoning tokens — deliberation is always estimated
...
/**
 * Create a Groq adapter with API key from environment variable.
 * C-SEC-P5-01/02: Key is read from env and NEVER logged.
 * CR-1: Key resolved once at creation.
 *
 * @param providerId - Provider identifier (defaults to 'groq')
 * @param time - Optional TimeProvider
 * @throws Error if GROQ_API_KEY env var is not set (message does NOT include key value)
 */
export function createGroqAdapterFromEnv(
  ...
  return createGroqAdapter(providerId, 'https://api.groq.com', apiKey, time);
}
