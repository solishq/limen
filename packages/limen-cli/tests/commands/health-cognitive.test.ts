...
beforeAll(async () => {
  // F-BR4-001: each dataDir must be initialized before any non-init command.
  // The bootstrap fallback to ~/.limen/master.key has been deleted
  // (trust-boundary violation remediation). Every isolated dataDir now
  // gets its own per-dir master.key generated by `init`.
  await runCli(`${GLOBAL_OPTS} init`);
  await runCli(`${EMPTY_OPTS} init`);
  await runCli(`${AGED_OPTS} init`);
  await runCli(`${CONFLICT_OPTS} init`);
