...
| DC-P1-202 | Convenience mission/agent not created during createLimen() eager init | 2: State consistency | DESIGN_PRINCIPLE | CBM | DS Decision 1, I-CONV-01 | **[A21]** Success: after createLimen(), remember() works immediately. Rejection: if init fails, convenience methods throw ENGINE_UNHEALTHY when called (non-fatal to engine -- direct ClaimApi remains usable). |
